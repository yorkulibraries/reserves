...
class ItemTest < ActiveSupport::TestCase
  ...
  should "not create an invalid eJournal Item" do
    ...
    assert ! build(:item, publication_date: nil, item_type: Item::TYPE_EJOURNAL).valid?, "Publication Date is required for ejournal"
    assert ! build(:item, journal_title: nil, item_type: Item::TYPE_EJOURNAL).valid?, "Journal Title is required for ejournal"
    assert ! build(:item, volume: nil, item_type: Item::TYPE_EJOURNAL).valid?, "Volume is required for ejournal"
    assert ! build(:item, page_number: nil, item_type: Item::TYPE_EJOURNAL).valid?, "Page Number is required for ejournal"
  end

  should "not create an invalid print_periodical Item" do
    # print_periodical
    assert ! build(:item, title: nil, item_type: Item::TYPE_PRINT_PERIODICAL).valid?, "Title is required for print_periodical"
    assert ! build(:item, author: nil, item_type: Item::TYPE_PRINT_PERIODICAL).valid?, "Author is required for print_periodical"
    assert ! build(:item, publication_date: nil, item_type: Item::TYPE_PRINT_PERIODICAL).valid?, "Publication Date is required for print_periodical"
    assert ! build(:item, journal_title: nil, item_type: Item::TYPE_PRINT_PERIODICAL).valid?, "Journal Title is required for print_periodical"
    assert ! build(:item, volume: nil, item_type: Item::TYPE_PRINT_PERIODICAL).valid?, "Volume is required for print_periodical"
    assert ! build(:item, page_number: nil, item_type: Item::TYPE_PRINT_PERIODICAL).valid?, "Page Number is required for print_periodical"
  end
  ...
  should "not create an invalid MULTIMEDIA Item" do
    ...
    assert ! build(:item, format: "something", item_type: Item::TYPE_MULTIMEDIA).valid?, "Format must be one of #{Item::MULTIMEDIA_FORMATS}"
  end
  ...
  should "not create an invalid MAP or Course Kit or Photocopy item" do
    ...
    assert ! build(:item, copyright_options: nil, item_type: Item::TYPE_PHOTOCOPY).valid?, "Copyright options are required for photocopy"
  ...
  should "Ensure callnumber is present if status is READY and item is provided by requestor" do
    ...
    assert build(:item, callnumber: "woot", provided_by_requestor: true, status: Item::STATUS_READY).valid?, "Should be valid"

    assert build(:item, callnumber: nil, provided_by_requestor: true, status: Item::STATUS_NOT_READY).valid?, "Should still be valid"
  end
  ...
  context "URL Validation" do
    ...
    should "invalid URL with URI" do
      ...
      result = my_url =~ URI::regexp(%w(http https))
