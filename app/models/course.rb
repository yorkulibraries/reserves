class Course < ApplicationRecord
  ...
  if Date.today.month > 8
    ACADEMIC_YEARS = Date.today.year..Date.today.year+1
  else
    ACADEMIC_YEARS = Date.today.year-1..Date.today.year
  end
  ...
  def get_value_from_code(position)
    self.code = "_______"   if self.code.blank?
    self.code.split("_")[position]
  end

  def insert_into_code(position, value)
    self.code = "_______"   if self.code.blank?
    broken = self.code.split("_")
