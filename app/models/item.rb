class Item < ApplicationRecord
  ...
  TYPES = [TYPE_BOOK, TYPE_EBOOK, TYPE_MULTIMEDIA, TYPE_COURSE_KIT, TYPE_PHOTOCOPY, TYPE_MAP, TYPE_EJOURNAL, TYPE_PRINT_PERIODICAL]
  ...
  validates_format_of :url, with: URI::regexp(%w(http https)), allow_blank: true, message: "URL is invalid. Please check again."

  # BOOK
  validates_presence_of :title, :author, :isbn, :publisher, if: Proc.new { |i| i.item_type == TYPE_BOOK }
  validates_inclusion_of :format, in: [FORMAT_BOOK, FORMAT_EBOOK], if: Proc.new { |i| i.item_type == TYPE_BOOK }

  # EBOOK
  validates_presence_of :title, :author, :publisher, if: Proc.new { |i|  i.item_type == TYPE_EBOOK }
  validates_inclusion_of :format, in: [FORMAT_BOOK, FORMAT_EBOOK], if: Proc.new { |i| i.item_type == TYPE_EBOOK }

  # MULTEMEDIA
  validates_presence_of :title, :author, :format, if: Proc.new { |i| i.item_type == TYPE_MULTIMEDIA }
  validates_inclusion_of :format, in: MULTIMEDIA_FORMATS, if: Proc.new { |i| i.item_type == TYPE_MULTIMEDIA }

  # PHOTOCOPY
  validates_presence_of :title, :author, :copyright_options, if:  Proc.new { |i| i.item_type == TYPE_PHOTOCOPY }
  validates_inclusion_of :format, in: [FORMAT_OTHER], if: Proc.new { |i| i.item_type == TYPE_PHOTOCOPY }

  # COURSE KIT
  validates_presence_of :title, :author, if:  Proc.new { |i| i.item_type == TYPE_COURSE_KIT }
  validates_inclusion_of :format, in: [FORMAT_OTHER], if: Proc.new { |i| i.item_type == TYPE_COURSE_KIT }

  # MAP
  validates_presence_of :title, :map_index_num, if:  Proc.new { |i| i.item_type == TYPE_MAP }
  validates_inclusion_of :format, in: [FORMAT_MAP], if: Proc.new { |i| i.item_type == TYPE_MAP }

  # EJOURNAL
  validates_presence_of :title, :author, :publication_date, :journal_title, :volume, :page_number, if:  Proc.new { |i| i.item_type == TYPE_EJOURNAL  }
  validates_inclusion_of :format, in: [FORMAT_ARTICLE], if: Proc.new { |i| i.item_type == TYPE_EJOURNAL }

  # PRINT_PERIODICAL
  validates_presence_of :title, :author, :publication_date, :journal_title, :volume, :page_number, if:  Proc.new { |i| i.item_type == TYPE_PRINT_PERIODICAL  }
  validates_inclusion_of :format, in: [FORMAT_ARTICLE], if: Proc.new { |i| i.item_type == TYPE_PRINT_PERIODICAL }
