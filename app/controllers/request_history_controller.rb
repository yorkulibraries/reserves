class RequestHistoryController < ApplicationController
  ...
  def index
    ...
    if request.xhr?
      render partial: "history_log", layout: false
    end
  ...
  def create
    ...
    respond_to do |format|
      if @aud.comment.blank?
        ...
        format.html { redirect_to history_request_path(@request), alert: 'Note not saved. Please contact application administrator' }
      end
    end
