class ItemsController < ApplicationController
  ...
  def index
    if params[:status] == Item::STATUS_DELETED
      @items = @request.items.deleted
    else
      @items = @request.items.active
    end
  end
  ...
  def change_status
    ...
    if status == Item::STATUS_DELETED && @request.status == Request::REMOVED
  ...
  private
    ...
    def item_params
      item_attributes = [ :id, :title, :author, :description, :callnumber, :isbn, :publication_date, :edition, :publisher, :loan_period, :provided_by_requestor,
