    ...
    class Connector
      ...
      def self.get_instance(class_name)
        ...
        if class_name != nil
          ...
          return Reserves::Ils::DummyConnector.new
        end
      end

      def create_reserve(item_id, reserve_location)

      end

      def create_or_link_course(course_id, item_id)

      end

      def link_user(user_id,item_id)
