class RequestMailer < ApplicationMailer
  ...
  def status_change(request, current_user)
    ...
    if request.requester_email.blank?
      @request_comm_email = request.requester.email
    else
      @request_comm_email = request.requester_email
    end

    bcc = Array.new
    if request.reserve_location.setting_bcc_request_status_change
  ...
  def expiry_notice(request, current_user=nil)
    ...
    if request.requester_email.blank?
      ...
      @request_comm_email = [request.requester_email, request.requester.email]
    end
